...
from common import is_compressed


class Unfrozenbbfreeze(object):
    def __init__(self, exe_file):
        self.exe_file = exe_file

        self.exe_file_path = os.path.abspath(self.exe_file)

        compression = is_compressed(self.exe_file_path)

        if compression and 'Microsoft Visual C++' not in compression:
            print("Binary compression used: {0}".format(compression))
            print("Before extracting and decompiling Python byte code from the binary you must decompressed it.")

            if 'UPX' in compression:
                print("\nFor UPX use:\n\n\t./upx -d <binary_name>")
    ...
    def get_archive_name(self):
        ...
        with open(self.exe_file_path, 'rb') as f:
            ...
            search_obj = re.search(r'\x00%s%c(\w+).zip%c%s\x00', source, re.M | re.I)

            if search_obj:
                archive_name = re.sub(r'[\x00%s%c\s]+', '', search_obj.group())
    ...
    def rename_main_script(self):
        ...
        orig_main_script_path = os.path.join(
            ...
            "__main__{0}__.pyc".format(os.path.splitext(os.path.basename(self.exe_file))[0].lower())
        )
...
if __name__ == "__main__":
    cx_Freeze_1 = Unfrozenbbfreeze(sys.argv[1])
